...
function gestionarEstadoSesion() {
  ...
  if (usuario && linkCuenta) {
    ...
    linkCuenta.href = "#";
    linkCuenta.onclick = cerrarSesion;

    // MAGIA ADMIN: Muestra el enlace si tiene rol de administrador
    if (usuario.es_admin === 1) {
      let nav = document.querySelector('nav');
      if (nav && !document.getElementById("linkAdmin")) {
        let adminLink = document.createElement("a");
        adminLink.href = "admin.html";
        adminLink.id = "linkAdmin";
        adminLink.textContent = "Panel Admin";
        adminLink.style.color = "var(--focus)";
...
function initAccountPage() {
  ...
  // Cambiar entre login y registro normal
  ...
  // --- MOSTRAR/OCULTAR LOGIN DE ADMINISTRADOR ---
  document.getElementById("btnShowAdminLogin")?.addEventListener("click", () => {
    loginSection.hidden = true;
    registerSection.hidden = true;
    adminToggleContainer.hidden = true;
    adminLoginSection.hidden = false;
  });

  document.getElementById("linkToNormalLogin")?.addEventListener("click", (e) => {
    e.preventDefault();
    adminLoginSection.hidden = true;
    loginSection.hidden = false;
    adminToggleContainer.hidden = false;
  });
  ...
  document.getElementById("btnLogin")?.addEventListener("click", async () => {
    ...
    try {
      ...
      if (response.ok) {
        // Guardamos usuario Y su rol
        localStorage.setItem("usuario", JSON.stringify({ nombre: data.nombre, es_admin: data.es_admin }));
  ...
  document.getElementById("btnAdminLogin")?.addEventListener("click", async () => {
    ...
    try {
      const response = await fetch('http://127.0.0.1:5000/api/login/admin', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (response.ok) {
        localStorage.setItem("usuario", JSON.stringify({ nombre: data.nombre, es_admin: data.es_admin }));
        alert("Acceso autorizado. Redirigiendo al panel de control...");
...
// ==========================================
document.addEventListener("DOMContentLoaded", () => {
  if (document.getElementById("pageAdmin")) {
    initAdminPage();
  }
});
...
async function initAdminPage() {
  const usuario = JSON.parse(localStorage.getItem("usuario"));
  // Seguridad: Echar a cualquiera que no sea admin
  if (!usuario || usuario.es_admin !== 1) {
    alert("Acceso denegado. Zona restringida a administradores.");
    window.location.href = "index.html";
    return;
  }
  ...
  const formAñadir = document.getElementById("formAñadirLibro");
  if (formAñadir) {
    formAñadir.addEventListener("submit", async (e) => {
      e.preventDefault();
      const nuevoLibro = {
        titulo: document.getElementById("adminTitulo").value,
        autor: document.getElementById("adminAutor").value,
        descripcion: document.getElementById("adminDesc").value,
        precio: document.getElementById("adminPrecio").value,
        categoria: document.getElementById("adminCat").value,
        imagen_url: document.getElementById("adminImg").value || "/static/uploads/default_book.png"
      };

      try {
        const res = await fetch("http://127.0.0.1:5000/api/admin/libros", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  try {
    ...
    libros.forEach(libro => {
      lista.innerHTML += `
                <div class="cart-item">
                    <div style="flex-grow: 1;">
                        <strong>${libro.titulo}</strong> <span class="muted">por ${libro.autor}</span>
                    </div>
                    <button class="btn btn-danger" onclick="borrarLibroAdmin(${libro.id})">Borrar</button>
                </div>
            `;
    });
  } catch (error) {
...
window.borrarLibroAdmin = async function (id) {
  if (confirm("¿Estás seguro de eliminar este libro de la base de datos?")) {
    try {
      await fetch(`http://127.0.0.1:5000/api/admin/libros/${id}`, { method: 'DELETE' });
      cargarLibrosAdmin(); // Refrescar lista
    } catch (error) {
      console.error(error);
    }
  }
