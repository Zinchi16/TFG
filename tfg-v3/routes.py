...
# ---------- ADMIN ----------
@api.route("/api/admin/libros", methods=["POST"])
def admin_add_book():
    data, status = f.añadir_libro_admin(request.json)
    return jsonify(data), status

@api.route("/api/admin/libros/<id>", methods=["DELETE"])
def admin_delete_book(id):
    data, status = f.borrar_libro_admin(id)
    return jsonify(data), status

@api.route("/api/login/admin", methods=["POST"])
def login_admin_route():
    data, status = f.login_admin(request.json)
    return jsonify(data), status